    ...
    while True:
        try:
            ...
            if min_val <= num <= max_val:
                return num
            print(f"O número deve estar entre {min_val} e {max_val} para esta largura de bits.")
        except ValueError:
...
def main():
    ...
    # Configuração inicial do tamanho da palavra
    while True:
        try:
    ...
    print(f"Frequência da CPU assumida: {CPU_FREQ_MHZ} MHz")
    ...
    while True:
        ...
        # Aplica o comportamento de N bits no resultado final
        res_nbits = para_signed_nbits(res_real, largura_bits)
        bin_nbits = para_binario_nbits(res_real, largura_bits)

        # Exibição dos resultados (padrão igual ao Serial do ESP32)
